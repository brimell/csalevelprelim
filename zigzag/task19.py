...
def GenerateEvaluations(NumbersAllowed, Targets):
    ...
    PossibleSolutions = [str(n) for n in NumbersAllowed if n in Targets]

    operators = ['+','-','*','/']
    
    # for each length 2..n of expression
    for r in range(2, len(NumbersAllowed)+1):
        # every ordering of r numbers
        for nums in permutations(NumbersAllowed, r):
            # every choice of operators (with repetition)
            for ops in product(operators, repeat=r-1):
                # build the flat expression
                expr = "".join(str(nums[i]) + ops[i] for i in range(r-1)) + str(nums[-1])
                try:
                    val = eval(expr)
                ...
                if isinstance(val, (int, float)) and val == int(val):
                    if int(val) in Targets and int(val) != -1:
                        PossibleSolutions.append(expr)
...
def PlayGame(Targets, NumbersAllowed, TrainingGame, MaxTarget, MaxNumber):
    ...
    while not GameOver:
        ...
        SuggestionMode = input("Would you like to recieve a suggestion this turn? (y/n)").lower() == "y"
        
        if SuggestionMode:
            print("")
            print("SUGGESTIONS:")
            for s in GenerateEvaluations(NumbersAllowed, Targets):
                print(s)
            print("")
